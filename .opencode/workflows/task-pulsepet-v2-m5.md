...
updatedAt: 2026-08-27T13:07:46+0800          # 每次写检查点必更新为当前时间（ISO 8601 含时区），不得沿用旧值
---
...
- [x] **文档滞后 2 项（来源 R2 committer 需求边界问题，轻度非代码缺陷）**：✅ 已清偿（2026-08-27 13:07 用户指示"现在就改"，supervised-coding 落笔）——实际修订范围 8 处：V2-DESIGN §5.6 重写（tab 两级筛选 + 修订注记 + 费用区注释小字同源滞后顺修）、§3.5 预留位前向注记、§5.11 P1-2 前向注记、§5.1/§5.7/§5.8×3 措辞对齐；V2-TEST-CASES TC-M5-03-2 费用区措辞 + TC-M5-10 目验清单同步；§5.0 裁定/§5.11 评审记录等历史留档不回改。待随下个流程文档提交点入库
...
- **交付后文档维护（2026-08-27 13:07，用户指示，supervised-coding 执行）**：12:58 后用户切换本地 develop 并拉取至 2ef7543，提交推送三项流程文件（检查点/事故报告/用例修订，commit `e3036bd`，2ef7543..e3036bd）；13:07 清偿"文档滞后 2 项"遗留（V2-DESIGN 6 处 + V2-TEST-CASES 2 处，见遗留事项勾销注记），改动在工作区待入库
