...
>
> **2026-08-20 裁定**：本清单各条目去向已定，见 [V2-SCOPE.md](./V2-SCOPE.md)——三/四全部 + 二（综合验收）→ **v0.1.3 维护版**；五-2（抢镜）→ **v2 M6**；五-1/3（心跳）→ **裁定不做**；七-1 随 v0.1.3 发版处置；七-2 拆仓 → **暂不拆**。各节内已逐条标注。
...
> **裁定（2026-08-20，见 V2-SCOPE §3/§4）**：#1/#3（心跳）→ **不做**（流式心跳落地后仅剩"探测插件进程存活"边缘用途）；#2（抢镜）→ **v2 M6**（用户确认真实多开，含多 agent 场景，气泡带 agent 标识）。
