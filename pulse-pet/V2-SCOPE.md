...
## 3. v2 主体（~5.5-7.5 周，五个里程碑）

### 3.1 M1：Claude Code 事件接入（~1-1.5 周）
...
- **单向 waiting-permission 语义**：宠物切 review 姿态提醒用户去看终端；用户在终端审批后经状态事件自动跟随更新。不做气泡内代答（见 §4.1）。

### 3.2 M2：前端 UI 优化 + Token 看板增强（~2 周封顶）
...
- **范围**：
  ...
  - 宠物气泡（提醒 / 会话汇报 / todo 派生文案各类形态）与宠物右键菜单；
...
- **约束**：zh/en 双语与字典键完备性测试保持；现有纯函数测试（bubble 文案 / token-chart 等）不破坏；Token 页重构时**预留 agent 筛选位**（数据维度 M4 接入，避免二次返工）；提醒 tab 在 M2 只做轻量翻新（落新设计系统），完整表单重做并入 M3 的「定时任务」合并 tab，避免双重返工。
- **排序理由**：置于功能扩展类里程碑之前——面板与气泡在 M3/M4/M5 均有改动，先立设计系统可让新功能直接落在新设计上，避免旧样式上开发再重绘的返工。
- **Token 看板增强**（用户 2026-08-21 需求，全部定案）：
...
- **宠物状态两层**：
  ...
  - 两层合并规则（agent 层优先、通用层兜底收尾）设计阶段定；与 M5 抢镜规则天然配套（例程会话与手头会话并行时靠最近活跃优先裁决）。
...
- **Token 页统一视图**：by agent 统计与展示（用户已裁定：token 可以 by agent 进行统计和展示）——统一页 + agent 筛选维度，跨 agent 汇总；UI 落在 M2 新设计系统上；M2 的 `token_stats_today` / 今日 preset / 模型筛选 / 项目名映射届时扩展 by agent 维度（CC transcript 的 message 含 model 字段，顺接）；例程会话（M3 spawn 的 opencode run）是否加来源标注，随本里程碑设计定。
- **会话汇报气泡**覆盖 Claude Code 会话（落 M2 重构后的新气泡组件）。

### 3.5 M5：多 agent × 多 session 抢镜（~0.5-1 周）
...
- **气泡带 agent 标识**：多 agent 后状态来源需可辨识（气泡/悬浮信息标注 opencode / claude-code；落 M2 重构后的新气泡组件）；与 M3 例程会话的抢镜协同（例程跑时恰逢手头会话活跃）随本里程碑设计定。
...
| 对话入口（opencode run 交互式） | 偏离桌宠定位（cc-haha 教训：桌宠沦为工作台一角）。**澄清**：M3 定时任务是后台 spawn + 事后汇报，无对话 UI，与本条裁定不冲突 |
...
1. **CC transcript 字段实测**：`~/.claude/projects/*.jsonl` 的 usage/模型字段结构需先 spike（M4 前置）；`message.updated` / `message.part.updated` 字段实测（v0.1.3 前置）。
2. **cost 口径**：CC 侧 cost 省略 vs 自配单价表估算（M4 设计定案，见 §3.4）。
3. **hooks 安装器幂等性**：`~/.claude/settings.json` 的 hooks 段结构比 opencode.json 的 plugin 数组复杂（按事件分组的命令数组），合并/卸载的 managed 标记方案需专门设计（M1 前置）。
4. **AgentAdapter 职责收口**（DESIGN §3.4 遗留）：v1 事件归一化在 TS 插件、token 读取在 Rust，跨两层；v2 接入第二 agent 时评估是否把 `tokenSource` 实现收口进 `AgentAdapter`（每个 adapter 自带 token 读取实现，Rust 侧只暴露通用 fs/JSONL 能力）。M4 设计时一并定。
...
7. **前端 UI（M2）设计输入**：设计语言与像素宠物的协调方案由 skill 辅助产出方向稿后用户拍板；样式技术取舍（维持手写 CSS vs 引入 Tailwind / 组件库——现零 UI 依赖、包体积敏感，v1 ~4MB 安装包是亮点）设计阶段定；frontend-design / impeccable skill 的安装在开工前确认（本机 github.com 网络不稳，可能需手动拷贝）。
8. **定时任务（M3）设计输入**：`reminders` 表迁移设计（action_type/action_params/schedule_kind 加列 + once 语义 + 与 todo 派生行共存）；exec 细节——超时默认值、Windows shell 差异（cmd/PowerShell）、`opencode run` 非交互模式下的权限行为（遇审批是卡死还是跳过，需 spike）、输出捕获上限；feature flag 禁用语义（隐藏 tab + 停派生提醒 + 数据保留的精确行为）；宠物状态两层合并规则（agent 层优先、通用层兜底收尾）；定点调度（daily-at/weekly-at/once）的到点判定与补跑宽限窗实现（MissedTickBehavior Skip 之上的精确判定）。
9. **tab 注册表形态（M2）**：核心 tab 与模块注册的边界（哪些属于核心不可关：Token/设置；哪些可关：Todo/定时任务）、注册表数据结构（前端静态注册 vs 消费 plugins 表）、与「最坏 6 个 tab」导航设计的配合。
10. **Token 看板增强（M2）实现细节**：悬停宠物 500ms 的防抖/关闭时机（mouseleave）与气泡既有 8s 自动消失逻辑的优先级；会话结束气泡追加「今日累计」——Rust idle hook 顺带查当日聚合、新鲜度护栏沿用 60s 口径；模型数据边界——session.model 可为 NULL（回退「未知模型」合并）、历史存在 `deepseek-v4-flash@max` 类带后缀 id（按原样归并）、probe-model 等 mock 模型是否过滤；会话标题兜底与列宽策略；JOIN project 表的性能影响（现有查询即开即关，预期可忽略）。
...
v0.1.3 维护版（含 TC-DONE 综合验收 + 发版）
  └→ v2 M1 Claude Code 事件接入（几乎无 UI 改动）
       └→ v2 M2 前端 UI 优化 + Token 看板增强（设计系统 + 面板 + 气泡 + tab 注册表/feature flag + 今日 token 三层通道 + 堆叠柱图/模型筛选/会话列表改造，skill 辅助）
            └→ v2 M3 定时任务（动作泛化 notify/exec + 「定时任务」合并 tab + 执行历史，落新设计系统）
                 └→ v2 M4 Token by agent（落新设计系统）
                      └→ v2 M5 抢镜规则（依赖 M1 多 agent 场景 + M3 例程会话；气泡 agent 标识落新气泡）
```

依赖关系：M2 依赖 M1 完成（CC 接入先稳定事件面再重构 UI）；M3 依赖 M2（新 tab 落新设计系统）；M4 依赖 M2（设计系统）与 M1（CC 数据源）；M5 依赖 M1（多 agent 场景）与 M3（例程会话参与抢镜），且受益于真实使用反馈。M4 与 M3 之间无强依赖，如需可互换。
