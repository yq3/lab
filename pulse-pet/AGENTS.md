...
- **例程（v2 M4，面板「例程」页）**：提醒与定时任务合并为一张列表（动作徽标 🔔 notify / ⚡ exec / 📋 todo 派生）；`reminders` 表迁移 003（`action_type` + `action_params` JSON + `schedule_kind` interval/daily/weekly/once）；exec 由 `action_exec.rs` spawn 命令，以 `task` **伪 agent** 驱动宠物状态（HTTP agent id 不得为 `"task"` 的根源）；daily/once 睡醒补跑宽限窗 15 分钟（`CATCHUP_WINDOW_MS`）；提醒气泡「稍后 10 分钟」snooze；托盘全局暂停期间 notify 不触发不补弹、exec 不跑不补（记 skipped）；**例程模板注册表**（routine-exec.md Part B，2026-08-30）：表单模板 chips 由 `src/lib/routine-templates.ts` 的 `ROUTINE_TEMPLATES` 驱动（OpenCode / Claude Code 两行起步），与 agent 注册表**正交**（不自动派生；加模板 = 该表一行 + i18n 键对，见 agent-onboarding checklist），`action_params` 新格式键 `tpl_agent` + `tpl_flags`（保存一律新格式、旧 `opencode_auto` 读兼容），执行层不感知 agent（Rust validate 仅宽松校验键形状）；**执行历史快照化**（routine-exec.md Part A/C，迁移 004/005，SCHEMA_VERSION=5）：`action_logs` 三快照列 `label`/`command`/`cwd`、分页 10 条/页——执行历史 = **执行时点快照**，规则改名/改命令/删除后历史仍显示当时内容（有意语义，勿当 bug 修）；**执行历史自动刷新**（V2-OPEN-ITEMS §二十七）：`action-logs://changed` 失效广播（exec 开始/终态/skipped 三写入点，Rust `ACTION_LOGS_CHANGED_EVENT` 常量）+ panel 例程页监听（`Tasks.tsx`，含 `reminder://trigger` 顺带刷「上次」列）。
...
**v2（V2-SCOPE/V2-DESIGN，2026-08 完成）**：M1 Claude Code 事件接入 + 接入管理 ✅ → M2 前端 UI 基础（设计系统 / 主题 / tab 注册表 / 气泡排队）✅ → M3 Token 看板增强 + 工具级气泡 ✅ → M4 例程（动作泛化 notify/exec）✅ → M5 Token by agent（CC transcript）✅ → M6 多 agent 抢镜 ✅；收尾批次见 V2-OPEN-ITEMS §十一~§二十五（宠物大小三档 / 反馈批次 F1~F16 / token 跨天归天 / Windows 图标 tile / 统计源状态行 / 品牌名统一 / exec 例程三缺陷修复 §二十二~§二十四 / 例程 exec 批次 §二十五——执行历史快照化 + 多 agent 模板注册表 + 执行上下文，见 routine-exec.md，均已实施）。后移：多屏与 Windows 剩余实机批次（V2-OPEN-ITEMS §六）、第三 agent 接入（codex，手册 `docs/v2/agent-onboarding.md`）。
