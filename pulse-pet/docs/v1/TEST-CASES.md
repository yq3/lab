...
- **状态**：用户明确取消，后续里程碑均不做。
- **说明**：原设计为可选"啾——砰"短音效（DESIGN §5.3，默认关、M5 评估）；2026-08-16 用户在 M5 开工确认时取消，不评估、不实现、不留资源文件。取消前已交付的 M4 无音频实现，无需回退。
...
- **前置**：同一 pet id 同时存在于 用户配置 → 内置宠物（blinking-kitty / wagging-doggy）→ `~/.codex/pets/` → `~/.petdex/pets/`。
- **步骤**：按序检查实际加载来源。
- **预期**：加载顺序为用户配置的 pet → 内置宠物 → `~/.codex/pets/` 扫描 → `~/.petdex/pets/` 扫描；找不到时逐级回退，最终必落到内置宠物（默认 blinking-kitty）。
...
- **前置**：M5 实现后；`~/.codex/pets/` 与 `~/.petdex/pets/` 各有素材、内置宠物可用。
- **步骤**：打开 panel → 设置 → "选择宠物"下拉。
- **预期**：
  1. 下拉列出：用户配置 pet → 内置宠物（**blinking-kitty 与 wagging-doggy 两只**）→ `~/.codex/pets/` 扫描结果 → `~/.petdex/pets/` 扫描结果（与加载顺序一致，见 TC-SP-06）；
  2. 切换后宠物立即重新加载并热替换 webview 帧（无需重启 App）；
  3. 选中素材损坏/非标准网格时，下拉项旁有回退提示（回退到上一可用素材或内置宠物，见 TC-SP-05）；
  4. 所有可选项均能被渲染，无空白宠物。

### TC-SP-12 内置宠物双可选（M5 补充，2026-08-16 用户需求）

- **前置**：M5 实现后，内置两只宠物（blinking-kitty 小猫 / wagging-doggy 小狗）均编译期内嵌。
- **步骤**：
  1. 无用户配置启动 → 默认加载 blinking-kitty（内置小猫，坐姿 + 单眨眼）；
  2. 面板"选择宠物"下拉切到 wagging-doggy（内置小狗，**简洁像素风、正面/偏正面、尾巴自然摆动**，风格与小猫一致不写实）→ 立即热替换；
  3. 切换回 blinking-kitty；重启 App 核对选择保留。
- **预期**：两只内置宠物均在下拉"内置"分组、均可渲染（无空白）；切换热替换生效（TC-SP-11②）；无配置默认 blinking-kitty（TC-SP-06 兜底）；选择持久化（TC-APP-12）；9 状态行映射两者一致（TC-SP-07 目视）。
