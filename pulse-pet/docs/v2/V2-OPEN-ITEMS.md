...
## 二十七、例程执行历史不自刷新（panel 无事件通知）+ 分页 15→10（2026-08-31，**已实施同日**）

> 来源：用户会话（2026-08-31）——「例程-执行历史，没有办法自动刷新，必须要先切换到别的 tab 页再切回来才行」；同会话追加裁定：执行历史分页大小 15→10。

**根因（三处叠加）**：① `Tasks.tsx` 仅在折叠面板开合 / 翻页 / 改筛选 / 本页 CRUD 后（`refresh()`）拉取执行历史，页面静置无任何刷新机制；② 后台 exec 完成只把结果事件 `pulsepet://task-result` `emit_to("pet")`（`action_exec.rs` `run_task`）——定向宠物窗口，panel 收不到，且 `action_logs` 另两处写入（`start_exec_run` 的 running 行 insert、`persist_skipped` 的 skipped 行 insert）完全没有事件；③ `Panel.tsx` 只挂载当前 tab——切走再切回 = remount `Tasks` → mount effect 重新拉数（即用户发现的"切 tab 才刷新" workaround 的原理）。

**方案一：新增 `action-logs://changed` 广播事件（失效信号，空 payload）**：

- Rust：`action_exec.rs` `TASK_RESULT_EVENT` 旁新增常量 `ACTION_LOGS_CHANGED_EVENT`，三处 emit——① `start_exec_run` insert running 成功 spawn 后；② `run_task` 终态回写块后（`emit_to("pet", TASK_RESULT_EVENT)` 气泡语义**不动**）；③ `reminder_scheduler.rs` `persist_skipped` 循环后整批一次（防逐行风暴）。**有意不覆盖**：R5 启动清理 / P1-2 退出处置两处 UPDATE（panel 未挂载或同亡，挂载即全量拉）。
- 前端（`Tasks.tsx`）：新增监听 effect——`isTauriRuntime()` 守卫 + 照 `Panel.tsx` PANEL_TAB_EVENT 的 alive-flag/unlisten 模式；`historyOpen/historyPage/historyFilter` 走 ref 防闭包过期。两个 listener：`action-logs://changed` → `load()` +（历史面板开时）`loadHistory(page, filter)`；`reminder://trigger`（既有全局广播，零 Rust 改动，用户裁定顺带修）→ `load()`（notify「上次」列实时；notify 不写 action_logs 无需刷历史）。事件名常量前端放 `lib/reminders.ts`（数据层，防 panel→pet 侧 `reminder-bridge.ts` 反向依赖）。
- 关联面核查：广播达三窗，pet 的 reminder-bridge 不监听本事件、fireworks 无监听 → 无副作用；事件量级 = 任务执行频次，无风暴；「试一试」走同一执行链天然覆盖；i18n 无新键、无 schema/迁移。

**方案二：分页 15→10**：`ACTION_LOG_PAGE_SIZE` 15→10（`reminder_scheduler.rs` 单一事实源，`page_size` 与 LIMIT 同源，前端 `totalPages` 用响应值无硬编码、逻辑零改动）。测试 `action_logs_crud_pagination_and_orphan_survival` 断言重算：61 条 10/页 → 第 1 页 10 条 / 第 2 页末条 `tail-41`（offset 10..20）/ 末页第 7 页 1 条 `tail-0` / 规则过滤页 10 条，注释同步。「15 条/页」字样随实施更新 6 处：`Tasks.tsx` / `reminders.ts` doc 注释、V2-DESIGN §4.7、V2-TEST-CASES TC-M4-16、README、AGENTS.md；`routine-exec.md` 为历史实施记录不回改。

**验证计划**：`cargo test`（src-tauri）+ `npm test` + `npx tsc --noEmit`；手动 E2E——`npm run tauri dev` 开例程页展开执行历史，建短间隔 exec 任务（或试一试）后**不切 tab**：running 行自动出现 → 终态自动翻转；分页 10 条/页；notify 触发后「上次」列自动更新；宠物窗 exec 结果气泡仍正常（回归）。

**实施记录（2026-08-31）**：已实施（用户批准后同日；工作区改动未 commit）。① Rust `action_exec.rs`：`TASK_RESULT_EVENT` 旁新常量 `ACTION_LOGS_CHANGED_EVENT`（`"action-logs://changed"`）+ 两处 emit（`start_exec_run` spawn 后 / `run_task` 终态回写后——pet 气泡 `emit_to("pet")` 原样不动）；② `reminder_scheduler.rs`：`persist_skipped` 循环后整批一次 emit（`Emitter` 模块级已有导入；R5 启动清理 / P1-2 退出处置按方案有意不覆盖）；③ 分页：`ACTION_LOG_PAGE_SIZE` 15→10 + 三处 doc 注释 + 测试 `action_logs_crud_pagination_and_orphan_survival` 断言重算（第 1/2 页 10 条、第 2 页末条 `tail-41`、末页改第 7 页 1 条 `tail-0`、过滤页 10）；④ 前端：`reminders.ts` 导出 `ACTION_LOGS_CHANGED_EVENT` / `REMINDER_TRIGGER_EVENT` 两常量（数据层，防 panel→pet 侧反向依赖）+ `Tasks.tsx` 监听 effect（`isTauriRuntime()` 守卫 + alive-flag/unlisten 模式，双 listener，`historyView` ref 每渲染同步防闭包过期）+ 头注释 10 条/页；⑤ 字样更新 4 处：V2-DESIGN §4.7 / V2-TEST-CASES TC-M4-16 / README / AGENTS.md（`routine-exec.md` 历史实施记录不回改）。**验证证据**：`cargo test` **379 passed + 4 ignored**（基线持平）/ `npm test` **464 passed**（33 文件，基线持平）/ `npx tsc --noEmit` 0 错；`npm run tauri build` 通过（`PulsePet.app` + `PulsePet_0.2.3_aarch64.dmg` 重建）。**实施偏差**：无（照清单落）。用户目验待办：例程页展开执行历史后不切 tab——running 行自动出现 → 终态自动翻转；分页 10 条/页；notify「上次」列实时；宠物窗 exec 结果气泡仍正常。

**Committer 审查（2026-08-31，§二十七）**：**APPROVED**（P0/P1/P2 均无，7 条 P3 备忘）。核查证据：三写入点 emit 位置/时机与方案一致（`start_exec_run` insert 失败路径 return false 不发——无写入即无失效；`run_task` 终态回写后、pet 气泡 `emit_to` 原样；`persist_skipped` 整批一次 + 空批守卫）；全仓 `action_logs` 写入点盘点无遗漏（`insert_action_log_running` / `finish_action_log_with` / `insert_action_log_skipped` prod 调用点各仅一处，R5 启动清理 / P1-2 退出处置为 UPDATE 且 panel 未挂载/同亡，与「有意不覆盖」一致，`tasks_skip_once` 不写 action_logs 无需 emit）；前端 effect 与 `Panel.tsx` 既有 alive-flag/unlisten 模式逐行同构、卸载无泄漏无迟到 setState，`load`/`loadHistory` `useCallback([])` deps 稳定；分页断言数值验算正确（61 条 10/页：tail-41 / 第 7 页 tail-0）且改回 15 四处翻红钉死新值，「15 条/页」残留仅剩有意不回改的 routine-exec.md 与本条目方案描述本身；三窗（pet `reminder-bridge` / fireworks / panelStore）无误消费，`reminder://trigger` 面板消费为纯增；常量放 `reminders.ts` 的依赖方向口径成立（`TASK_RESULT_EVENT` 前端定义在 pet 侧 bridge，Tasks.tsx 此前不 import 之）；实施记录①~⑤与 diff 逐条相符。**P3 备忘（不阻塞，均未改）**：① `historyView.current` 渲染期赋值（幂等无实害，可改 useEffect 同步）；② 监听 import 链无 `.catch`（listen 失败静默不自刷新，与 `Panel.tsx` 既有模式一致非回归）；③ 双 listen 第二个注册失败时首个 listener 泄漏（仅 Tauri 事件系统故障触发，概率极低）；④ `persist_skipped` 批内全 notify 型（不写 action_logs）时仍发一次广播——一次多余刷新，无害（可加「批内含 exec 型」判定）；⑤ `"reminder://trigger"` 字面量三处并存（reminders.ts / reminder-bridge.ts / Rust 侧），可后续统一；⑥ 实施记录⑤「字样更新 4 处」与方案"6 处"计数行文小账（Tasks.tsx 头注释计在④、reminders.ts doc 未单独列名，实际 6 处全改）；⑦ 本批无新增测试钉 emit 行为（前端 vitest 无 Tauri 事件系统难测、Rust mock runtime 成本高，现有全绿 + 语义简单，不强制）。
...
- [x] **§二十七 例程执行历史不自刷新（panel 无事件通知）+ 分页 15→10**：✅ 已实施 2026-08-31（方案定稿 + 用户批准，同日实施）——`action-logs://changed` 失效广播（`action_exec.rs` 新常量 + `start_exec_run`/`run_task` 两处 emit、`persist_skipped` 整批一处；R5/P1-2 有意不覆盖；pet 气泡 `emit_to` 不动）+ `Tasks.tsx` 监听 effect（`historyView` ref 防闭包过期，顺带 `reminder://trigger` 刷「上次」列；常量放 `reminders.ts`）+ `ACTION_LOG_PAGE_SIZE` 15→10（测试断言重算：10 条/`tail-41`/末页第 7 页）。基线 cargo **379 passed + 4 ignored** / npm **464** 原样 / tsc 0 错；`tauri build` 通过（app + dmg 重建）。完整留痕：§二十七；用户目验待办（不切 tab 自动刷新 + 10 条/页 + 气泡回归）
